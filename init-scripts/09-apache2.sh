...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set webroot
WWW_ROOT_DIR="/usr/share/webapps/ampache/public"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
ampache_secret_key="$(__random_password 32)"
ampache_conf_file="/config/ampache/ampache.conf"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__update_conf_files() {
  ...
  [ -f "$USER_FILE_PREFIX/root/mysql__name" ] && local db_user_name="$(<"$USER_FILE_PREFIX/root/mysql__name")" || local db_user_name="unset"
  [ -f "$USER_FILE_PREFIX/root/mysql__name" ] && local db_user_pass="$(<"$USER_FILE_PREFIX/root/mysql__name")" || local db_user_pass="unset"
  [ -f "$ROOT_FILE_PREFIX/root/mysql__name" ] && local db_root_name="$(<"$ROOT_FILE_PREFIX/root/mysql__name")" || local db_root_name="unset"
  [ -f "$ROOT_FILE_PREFIX/root/mysql__name" ] && local db_root_pass="$(<"$ROOT_FILE_PREFIX/root/mysql__name")" || local db_root_pass="unset"
  [ -f "$USER_FILE_PREFIX/ampache_secret.key" ] && ampache_secret_key="$(<"$USER_FILE_PREFIX/ampache_secret.key")" || echo "$ampache_secret_key" >"$USER_FILE_PREFIX/ampache_secret.key"
  ...
  # custom commands
  if [ -f "$ampache_conf_file" ]; then
    __sed "s|REPLACE_DB_HOST|127.0.0.1|g" "$ampache_conf_file"
    __sed "s|REPLACE_SECURITY_CODE|$secret_key|" "$ampache_conf_file"
    [ -n "$root_user_pass" ] && __sed 's|REPLACE_PASSWORD|'$root_user_pass'|g' "$ampache_conf_file"
    __file_copy "/config/ampache/." "/var/www/ampache/config/"
  else
    __sed "s|REPLACE_DB_HOST|127.0.0.1|g" "$WWW_DIR/config/ampache.cfg.php.dist"
    __sed "s|REPLACE_SECURITY_CODE|$secret_key|" "$WWW_DIR/config/ampache.cfg.php.dist"
    [ -n "$root_user_pass" ] && __sed 's|REPLACE_PASSWORD|'$root_user_pass'|g' "$WWW_DIR/config/ampache.cfg.php.dist"
  fi
  __replace "REPLACE_DATABASE_USER" "$user_name" "/config/ampache/ampache.conf"
  __replace "REPLACE_DATABASE_PASS" "$user_name" "/config/ampache/ampache.conf"
  __replace "REPLACE_DATABASE_USER" "$user_name" "/config/ampache/ampache.conf"
...
__post_execute() {
  ...
  (
    sleep 20
    DATABASE_NAME="ampache" DB_USER_NAME="$user_name" DB_USER_PASS="$user_pass" ADMIN_USER="$root_user_name" ADMIN_PASS="$root_user_pass" db_init_mysql
  ) |& tee -p -a "$LOG_DIR/init.txt" &>/dev/null &
  return $exitCode
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
